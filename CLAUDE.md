# Wiki — working conventions

Conventions for working in this repo. The audience is your team; treat the repo as private
to the team unless you've decided otherwise. File by **what a doc is**.
...
Tie-breakers: policy = the rule, guide = the steps to comply. Guide = build/learn,
runbook = operate/recover under pressure, incident = the retrospective written *after*
something broke.
...
- `kebab-case.md`. A simple topic is one file; a system with sub-pages is a folder with an
  `index.md`.
- Every doc starts from [`_template.md`](_template.md) — required frontmatter: `title`,
  `description`, `owner`, `updated` (`YYYY-MM-DD`). `owner` is a person or team; **a doc with
  no owner rots.**
...
If `main` moved while a PR was open, the conflict is almost always in
